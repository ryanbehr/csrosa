    ...
    public class StreamsUtil
    {
        ...
        public static sbyte[] readFromStream(System.IO.Stream in_Renamed, int len)
        {
            ...
            if (len >= 0)
            {
                ...
                while (read < len)
                {
                    int k = in_Renamed is BufferedStream ? ((BufferedStream)in_Renamed).Read((byte[])(Array)data, read, len - read) : in_Renamed.Read((byte[])(Array)data, read, len - read);
                    if (k == -1)
                        break;
                    read += k;
                }
            ...
            else
            {
                ...
                data = (sbyte[])(Array)buffer.ToArray();
                read = data.Length;
            }
